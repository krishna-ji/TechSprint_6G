...
🚀 UNIFIED SCRIPT - All-in-One Data Pipeline and Visualization Suite

This single script handles:
1. Dataset generation (training + test data)
2. Scientific validation plots
3. Enhanced analytics visualizations (collision analysis, autocorrelation, spectrograms, etc.)

All visualizations are generated automatically during pipeline execution.
No separate scripts needed - just run: python src/data_pipeline.py
...
Generates heterogeneous IoT traffic using Markov-Modulated Poisson Process (MMPP)
with three device classes: Critical (URLLC), Delay-Tolerant (mMTC), High-Throughput (eMBB-IoT).
"""
...
class IoTTrafficConfig:
    """
    Configuration for IoT-specific traffic generation.
    
    Implements heterogeneous device classes per ETSI EN 303 645 and ITU-R M.2083-0.
    
    Attributes
    ----------
    n_channels : int
        Number of frequency channels in spectrum (default: 20)
    time_steps_train : int
        Training dataset duration in time slots (default: 10000)
    time_steps_test : int
        Testing dataset duration in time slots (default: 2000)
    n_devices : int
        Total IoT devices in simulation (6G massive IoT scale, default: 1000)
    device_distribution : dict
        Proportion of each IoT device class
    type_a : dict
        Critical IoT (URLLC) parameters - medical sensors, industrial control
    type_b : dict
        Delay-tolerant IoT (mMTC) parameters - smart home, environmental sensors
    type_c : dict
        High-throughput IoT (eMBB-IoT) parameters - video surveillance, AR/VR
    """
